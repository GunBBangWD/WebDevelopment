...
SELECT DISTINCT JOB_ID , MAX(SALARY) OVER(PARTITION BY JOB_ID) JOBMAX
...
--예상 답 2 번
SELECT DISTINCT EXTRACT(YEAR FROM HIRE_DATE) ,COUNT(*) OVER(PARTITION BY EXTRACT(YEAR FROM HIRE_DATE)),
(SELECT COUNT(*) FROM EMPLOYEES) FROM EMPLOYEES 
WHERE EXTRACT(YEAR FROM HIRE_DATE)IN ('2001','2003','2005','2006');

SELECT TO_CHAR(HIRE_DATE,'YYYY'), COUNT(*) FROM EMPLOYEES e, DEPARTMENTS d 
WHERE E.DEPARTMENT_ID = D.DEPARTMENT_ID 
AND TO_CHAR(HIRE_DATE,'YYYY') IN ('2005','2006','2007','2008') 
...
SELECT EMPLOYEE_ID, LAST_NAME FROM EMPLOYEES 
WHERE DEPARTMENT_ID IN(SELECT * FROM UIN);
...
CREATE OR REPLACE VIEW UIN2
AS SELECT DEPARTMENT_ID FROM DEPARTMENTS WHERE LOCATION_ID=1700;

SELECT LAST_NAME, DEPARTMENT_ID, JOB_ID FROM EMPLOYEES
WHERE DEPARTMENT_ID IN (SELECT * FROM UIN2);
...
IN (SELECT DEPARTMENT_ID FROM DEPARTMENTS WHERE DEPARTMENT_NAME=INITCAP('EXECUTIVE'));
...
--       근무하는 부서에서 근무하는 사원들의 사번, LAST_NAME 및 연봉을 조회한다.
CREATE OR REPLACE VIEW VI3
AS SELECT DEPARTMENT_ID FROM EMPLOYEES WHERE LAST_NAME LIKE '%u%';

SELECT EMPLOYEE_ID, LAST_NAME, SALARY FROM EMPLOYEES
WHERE SALARY>(SELECT TRUNC(AVG(SALARY)) FROM EMPLOYEES)
AND DEPARTMENT_ID IN(SELECT DEPARTMENT_ID FROM VI3);

-- 88. ST_CLERK 인 직업 ID 를 가진 사원이 없는 부서 ID 를 조회한다.
--       NULL 값은 제외한다. (NOT EXISTS 사용)

SELECT DEPARTMENT_ID FROM EMPLOYEES E GROUP BY DEPARTMENT_ID HAVING
NOT EXISTS(SELECT 1 FROM EMPLOYEES WHERE E.DEPARTMENT_ID 
IN(SELECT DISTINCT DEPARTMENT_ID FROM EMPLOYEES WHERE JOB_ID='ST_CLERK')) AND DEPARTMENT_ID IS NOT NULL;
...
SELECT DEPARTMENT_ID FROM EMPLOYEES E GROUP BY DEPARTMENT_ID HAVING
NOT EXISTS(SELECT 1 FROM EMPLOYEES WHERE E.DEPARTMENT_ID 
NOT IN(SELECT DISTINCT DEPARTMENT_ID FROM EMPLOYEES WHERE JOB_ID='ST_CLERK')) AND DEPARTMENT_ID IS NOT NULL;
...
SELECT DISTINCT C.COUNTRY_ID FROM LOCATIONS L
RIGHT JOIN COUNTRIES C ON L.COUNTRY_ID=C.COUNTRY_ID
LEFT JOIN DEPARTMENTS D ON L.LOCATION_ID=D.LOCATION_ID
GROUP BY C.COUNTRY_ID, D.DEPARTMENT_ID HAVING NOT EXISTS(SELECT 1
FROM DEPARTMENTS ED GROUP BY DEPARTMENT_ID HAVING D.DEPARTMENT_ID=ED.DEPARTMENT_ID);
...
RIGHT JOIN COUNTRIES C ON L.COUNTRY_ID=C.COUNTRY_ID
LEFT JOIN DEPARTMENTS D ON L.LOCATION_ID=D.LOCATION_ID
GROUP BY C.COUNTRY_ID, D.DEPARTMENT_ID HAVING EXISTS(SELECT 1
FROM DEPARTMENTS ED GROUP BY DEPARTMENT_ID HAVING D.DEPARTMENT_ID=ED.DEPARTMENT_ID);
...
JOIN (SELECT DEPARTMENT_ID, LOCATION_ID FROM DEPARTMENTS) D 
USING (DEPARTMENT_ID) WHERE D.LOCATION_ID = 1700 ORDER BY SALARY;

SELECT LAST_NAME,DEPARTMENT_ID FROM EMPLOYEES 
WHERE SALARY IN (SELECT SALARY,COMMISSION_PCT FROM EMPLOYEES E 
JOIN (SELECT DEPARTMENT_ID, LOCATION_ID FROM DEPARTMENTS) D 
USING (DEPARTMENT_ID) WHERE D.LOCATION_ID = 1700 ORDER BY SALARY);

(CASE WHEN 
SALARY IN (SELECT SALARY FROM EMPLOYEES E 
JOIN (SELECT DEPARTMENT_ID, LOCATION_ID FROM DEPARTMENTS) D 
USING (DEPARTMENT_ID) WHERE D.LOCATION_ID = 1700 ORDER BY SALARY)  THEN 
(CASE WHEN
SALARY IN (SELECT COMMISSION_PCT FROM EMPLOYEES E 
JOIN (SELECT DEPARTMENT_ID, LOCATION_ID FROM DEPARTMENTS) D 
USING (DEPARTMENT_ID) WHERE D.LOCATION_ID = 1700 ORDER BY SALARY) THEN SALARY
))



SELECT * FROM EMPLOYEES E JOIN (SELECT DEPARTMENT_ID, LOCATION_ID FROM DEPARTMENTS) D 
USING (DEPARTMENT_ID) 
WHERE NOT EXISTS(SELECT 1 FROM EMPLOYEES EE 
WHERE D.LOCATION_ID = 1700 AND EE.SALARY=E.SALARY AND EE.COMMISSION_PCT=E.COMMISSION_PCT);
...
SELECT D.DEPARTMENT_NAME, E.FIRST_NAME, E.SALARY, (CASE 
WHEN E.COMMISSION_PCT IS NULL THEN 0.0
ELSE E.COMMISSION_PCT 
END)  COMMISSION
FROM EMPLOYEES E JOIN DEPARTMENTS D USING(DEPARTMENT_ID);

SELECT D.DEPARTMENT_NAME , E.FIRST_NAME , E.SALARY , E.COMMISSION_PCT FROM EMPLOYEES E
JOIN DEPARTMENTS D ON E.DEPARTMENT_ID=D.DEPARTMENT_ID
WHERE E.COMMISSION_PCT  IS NOT NULL;

--Q2 부서명, 직책, 직원명, 입사일을 갖는 뷰를 작성하시오.
CREATE OR REPLACE VIEW DJEH_VIEW
AS SELECT DEPARTMENT_NAME, E.JOB_ID, E.FIRST_NAME, E.HIRE_DATE FROM EMPLOYEES E
JOIN DEPARTMENTS D USING(DEPARTMENT_ID);

SELECT * FROM DJEH_VIEW;
...
SELECT D.*,A.EMP_CNT FROM DEPARTMENTS D,
...
BEGIN
    ...
    ELSIF UPDATING THEN
        IF :OLD.DEPARTMENT_ID<>:NEW.DEPARTMENT_ID THEN -- 누락된 THEN 추가
            UPDATE DEPARTMENTS2
            SET EMP_CNT=EMP_CNT+1
            WHERE DEPARTMENT_ID = :NEW.DEPARTMENT_ID;
            UPDATE DEPARTMENTS2

...
SELECT E.EMPLOYEE_ID 사원번호, D.DEPARTMENT_NAME 부서명, J.JOB_TITLE 직무명 FROM EMPLOYEES E
LEFT JOIN DEPARTMENTS D ON E.DEPARTMENT_ID=D.DEPARTMENT_ID
LEFT JOIN JOBS J ON E.JOB_ID=J.JOB_ID;

-- 21. 모든 사원들의 사원번호, 부서명, 직무명, 도시명을 조회하라.
SELECT E.EMPLOYEE_ID 사원번호, D.DEPARTMENT_NAME 부서명, J.JOB_TITLE, L.CITY 도시명 FROM EMPLOYEES E
LEFT JOIN DEPARTMENTS D ON E.DEPARTMENT_ID=D.DEPARTMENT_ID
LEFT JOIN JOBS J ON E.JOB_ID=J.JOB_ID
LEFT JOIN LOCATIONS L ON D.LOCATION_ID=L.LOCATION_ID;
...
SELECT * FROM EMPLOYEES WHERE MANAGER_ID IN(SELECT MANAGER_ID FROM DEPARTMENTS);
...
SELECT DISTINCT D2.DEPARTMENT_NAME,D.SALMIN,D.SALMAX,D.CNT 
FROM EMPLOYEES E JOIN 
(SELECT DEPARTMENT_ID DEPTNOM, MIN(SALARY) SALMIN, MAX(SALARY) SALMAX, COUNT(*) CNT
FROM EMPLOYEES GROUP BY DEPARTMENT_ID
) D ON E.DEPARTMENT_ID=D.DEPTNOM 
JOIN (SELECT DEPARTMENT_ID,DEPARTMENT_NAME FROM DEPARTMENTS) D2 ON E.DEPARTMENT_ID=D2.DEPARTMENT_ID
WHERE E.SALARY=D.SALMIN OR E.SALARY=D.SALMAX;

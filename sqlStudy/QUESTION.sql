...
-- 66-1. 시애틀에 사는 사람 중 커미션을 버는 모든 사람들의 LAST_NAME, 부서 명, 지역 ID 및 도시 명을 조회한다. Seattle
SELECT E.LAST_NAME, D.DEPARTMENT_NAME, D.LOCATION_ID, L.CITY FROM EMPLOYEES E
JOIN DEPARTMENTS D ON E.DEPARTMENT_ID=D.DEPARTMENT_ID
JOIN LOCATIONS L ON D.LOCATION_ID=L.LOCATION_ID 
...
SELECT D.DEPARTMENT_NAME 부서명, D.LOCATION_ID 위치ID, COUNT(E.EMPLOYEE_ID) 사원총수, TRUNC(AVG(E.SALARY),2) 평균연봉 FROM EMPLOYEES E
